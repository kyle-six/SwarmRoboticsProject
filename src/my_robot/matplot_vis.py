...
def update(frame):
    ...
    ax.set_ylim(-15, 15)
    ax.set_aspect('equal')
    ax.set_title(f'Step {frame}: Smart Dispersion\nTarget zone: {lower_threshold}-{upper_threshold} units')
    ax.grid(True)
    ...
    for robot in robots:
        ...
        ax.plot(robot.x, robot.y, marker, color=robot.color, markersize=10)
        ...
        ax.text(robot.x, robot.y + 0.5, status, 
               color=robot.color, ha='center', fontsize=8)
    
    # Display distances
    for (i,j), dist in distances.items():
        mid_x = (robots[i].x + robots[j].x)/2
        mid_y = (robots[i].y + robots[j].y)/2
        ax.text(mid_x, mid_y, f"{dist:.2f}", ha='center', fontsize=8,
               bbox=dict(facecolor='white', alpha=0.7, edgecolor='none'))

fig, ax = plt.subplots(figsize=(10, 8))
ani = FuncAnimation(fig, update, frames=max_steps, interval=200, repeat=False)
plt.show()